...
def AASM_Rules(raw,metadata,result):
    ...
    for epoca, deteccion in enumerate(Reevaluacion_pd): # epoca indicaria el momento de la epoca que ocurrio osea seria el idx, probar, y deteccion indica el momento que ocurrio la deteccion
        Puntos = [0] * 5 # idx : 0 es W idx 1,2,3 es n1, n2, n3 respectivamente, idx 4 es REM
        if deteccion!=0:
            print('ingreso, en la epoca :', epoca)

            start_time = deteccion  # las detecciones siempre vana  estar redondeadas en 30 segundos, proque se marcan al inciio de cada epoca
            ...
            if metadata['channels']['eeg']['occipital']:
                for canal_O in  metadata['channels']['eeg']['occipital']:
                    ...
                    if(Porcentaje_de_densidad>0.5) :
                        ...
                        Puntos[0] += 1
                        ##  ME QUEDO CON LOS EVENTOS MAS LARGOS QUE 0,5 MSEG ?  O LOS ELIMINO ... EVALUAR ESTO Y VER Q HACER
            ...
            if len(eventos_en_rango_spindle) != 0 or (len(eventos_en_rango_sw)!= 0 and np.sum(duracion__en_rango_sw) < 0.2*30):  #len(eventos_en_rango_spindle)< 4# pongo menor a 4 porque suponiendo que duran 1.5 en promedio con tener mas de 4 ya deberia considerarse SW de N3 y no Complejo K, CONSULTAR SI TA BIEN ASI 
                print('Suma puntos para ser N2')                                             # se pone 0.2*30 para descartar que sea N3 dado que muchos kc juntos correspondena  etapa n3`
                Puntos[2] += 1 
                """
            ...
            """
            if result['GSSC'][1][epoca-1] == 4 and (len(eventos_en_rango_spindle) != 0 or (len(eventos_en_rango_sw)!= 0)): # osea indica que es r la epoca anterior 
                print('suma puntos para ser rem')
                Puntos[4] += 1
...
def Final_Vote(results, raw):
    ...
    if sum(No_concidencias) > sum(anotaciones_gssc)*0.3 :  # en estas lineas de codigo se implementa la estrategia de usar los pesos de gssc
        ...
        ################# REVISAR QUE CUANDO USO FIND EVENTS NO LOGRO OBTENER LOS EVENTOS TAL CUAL QUISIERA ####################
...
def sleep_stage_classification_file(file_data, metadata, classifiers, detectores):
    ...
    Args:
        ...
        detectores /metadata (dict): dicionario con los nombres y las detectores que voy a usar para detectar eventos particulas del sueño
        debe tener el siguiente formato
        dict _ {
        detectors : []
        }
